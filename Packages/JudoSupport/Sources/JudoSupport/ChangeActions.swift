...
public extension Repository {
    func new(jujutsu: Jujutsu, changes: [ChangeID] = []) async throws {
        try await runner.run(subcommand: "new", arguments: changes.map(\.description), invalidatesCache: true)
    }

    func undo(jujutsu: Jujutsu) async throws {
        try await runner.run(subcommand: "undo", arguments: [], invalidatesCache: true)
    }

    func abandon(jujutsu: Jujutsu, changes: [ChangeID]) async throws {
        let arguments = changes.map(\.description)
        try await runner.run(subcommand: "abandon", arguments: arguments, invalidatesCache: true)
    }

    func squash(jujutsu: Jujutsu, changes: [ChangeID]) async throws {
        let arguments = ["-r", changes.map(\.description).joined(separator: " | ")]
        try await runner.run(subcommand: "squash", arguments: arguments, invalidatesCache: true)
    }
    ...
    func describe(jujutsu: Jujutsu, changes: [ChangeID], description: String) async throws {
        ...
        try await runner.run(subcommand: "describe", arguments: arguments, invalidatesCache: true)
    }
    ...
    func rebase(jujutsu: Jujutsu, from: [ChangeID], to: ChangeID) async throws {
        ...
        try await runner.run(subcommand: "rebase", arguments: arguments, invalidatesCache: true)
    }
}
